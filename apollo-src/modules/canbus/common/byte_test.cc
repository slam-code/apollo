...
TEST(ByteTest, CopyConstructor) {
  unsigned char byte_value = 0xFF;// 8个１：【１１１１１１１１】
...
TEST(ByteTest, SetBit) {
  unsigned char byte_value = 0xFF;// 8个１：【１１１１１１１１】
  Byte value(&byte_value);
  value.set_bit_0(1); //设置为0.从右往左数第１个。begin:0
  EXPECT_EQ(0xFD, value.get_byte());//[11111101]
  value.set_bit_0(7); //第７位为设置为０，即[01111101]
  EXPECT_EQ(0x7D, value.get_byte());
  value.set_bit_1(7);
  EXPECT_EQ(0xFD, value.get_byte());//[11111101]
  value.set_value(0x77);            //01110111
  value.set_bit_1(0);               //01110111
  EXPECT_EQ(0x77, value.get_byte());

                                 //01110111
  EXPECT_TRUE(value.is_bit_1(0));//       1
  EXPECT_TRUE(value.is_bit_1(1));//      1
  EXPECT_FALSE(value.is_bit_1(3));//   0
  EXPECT_TRUE(value.is_bit_1(6));// 1
  EXPECT_FALSE(value.is_bit_1(7));//0
}

TEST(ByteTest, SetValue) {
  unsigned char byte_value = 0x1A;  //   【00011010】
  Byte value(&byte_value);
  value.set_value(0x06, 3, 3); //0x06是110，将byte的第３位起设置为0x06.
  EXPECT_EQ(0x32, value.get_byte());//结果是1100101
...
TEST(ByteTest, ByteToString) {
  unsigned char value = 0x34;　　　　　　　　　　　　　　　　　　　　//16进制表示的52
  
  EXPECT_EQ("34", Byte::byte_to_hex(value));　　　　　//转为16进制的字符
  EXPECT_EQ("00110100", Byte::byte_to_binary(value)); //10进制：52
