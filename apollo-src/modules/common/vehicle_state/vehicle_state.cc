...
Eigen::Vector2d VehicleState::EstimateFuturePosition(const double t) const {
  ...
  double v = linear_v_;
  if (gear_ == canbus::Chassis::GEAR_REVERSE) {//倒车，速度翻转。
    v = -linear_v_;
  }
  // Predict distance travel vector
  if (std::fabs(angular_v_) < 0.0001) {//直线
    vec_distance[0] = 0.0;
    vec_distance[1] = v * t;
  } else {                             //有弯
...
Eigen::Vector2d VehicleState::ComputeCOMPosition(
  ...
  Eigen::Vector3d v(0.0, rear_to_com_distance, 0.0); //只有y方向有意义。
  Eigen::Vector3d pos_vec(x_, y_, z_);
