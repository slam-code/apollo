 ...
 * IdentifierType to ProductCreator, by default std::unordered_map
 ...
 ProductCreator：函数指针。形参是空。返回值是指针，指向抽象class。
  MapContainer = std::map<IdentifierType, ProductCreator>：按照IdentifierType存储需要创建的对象。创建方法是ProductCreator
...
class Factory {
 public:
   ...
   * @param id The identifier of the class we which to instantiate
   ...
   现在开始创建对象。使用IdentifierType对应的函数指针创建特点对象
   */
  std::unique_ptr<AbstractProduct> CreateObject(const IdentifierType &id) {
    ...
    if (id_iter == producers_.end()) {
      ...
      //调用函数()运算符,此返回的是裸指针，用智能指针包装一下再返回。
    }
  }
