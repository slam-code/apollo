...
void Monitor::Publish(const std::vector<MessageItem> &messages) const {
  ...
  MonitorMessage monitor_msg;//把vector里的messages整合到这里。

  for (const auto &msg_item : messages) {//对每一个消息项
    MonitorMessageItem *monitor_msg_item = monitor_msg.add_item();//添加一元素。
